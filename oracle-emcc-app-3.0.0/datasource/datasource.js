            ...
            OracleEMCDatasource = (function () {
                ...
                */
                OracleEMCDatasource.prototype.buildQueryParameters = function (options) {
                    ...
                    // queryResources = queryResources.filter(t => t === undefined || (!t.isAnyValueEmpty() && !t.hide));
                    queryResources = queryResources.filter(function (t) { return t !== undefined && !t.hide; });
                ...
                */
                OracleEMCDatasource.prototype.doRequest = function (options) {
                    ...
                    return this.backendSrv.datasourceRequest(options)
                        ...
                        if (error && error.data && error.data.message && error.data.message.indexOf('<div class="preloader__text">Loading Grafana</div>') !== -1) {
                            throw ({
                                statusText: "Enterprise Manager Cloud Control is not currently available. The EMGC_OMS server where Cloud Control runs, or some other component, might be down",
                                status: 404,
                                info: "EMC_DOWN"
                ...
                */
                OracleEMCDatasource.prototype.metricFindQuery = function (options) {
                    var handleErr = function (err) {
                        ...
                        console.log('Error occurred attempting to fetch template variable values: ', err);
                    };
                    options = JSON.parse(options);
                    if (options.type === 'targettype') {
                        return this.getTargetTypes().catch(handleErr);
                    }
                    else if (options.type === 'target') {
                        options.targettype = this.templateSrv.replace(options.targettype, null, 'regex');
                        return this.getTargetValues(options.targettype).catch(handleErr);
                    }
                        ...
                        return this.getNamedCredentials(options.targettype, options.target).catch(handleErr);
                    }
                };
                /**
